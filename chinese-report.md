...
## 摘要
本研究針對齒輪製程所蒐集之三軸振動訊號進行特徵工程與神經網路微調，旨在提升壞品偵測的準確性。透過擴充手動分析所得的 Top 10 特徵為 39 維延伸特徵集，並導入類別權重與驗證集閾值調校，最終多層感知器模型於測試集達成 98.4% 準確率、97.9% 精確率與 100% 召回率，驗證閾值為 0.16。另以 5 折交叉驗證驗證「最佳模型」在整體資料上的泛化性，平均測試準確率 97.1%、F1 0.981、平衡準確率 0.943，證明流程穩定可靠。本報告整合資料處理、特徵萃取、模型訓練與視覺化檢視流程，作為後續部署與研究延伸之參考。
...
- 指令：
  ```bash
  python cross_validate_feature_mlp.py --dataset datasets/cw_extended_features.csv \
    --k 5 --epochs 200 --batch-size 32 \
    --dropout 0.2 --hidden-sizes 512 256 128 \
    --learning-rate 3e-4 --val-ratio 0.15 --quiet \
    --output reports/cw_extended_feature_kfold.json
  ```
- 報告：`reports/cw_extended_feature_kfold.json` 收錄每個折的訓練/驗證/測試指標、類別權重與實際訓練輪次，便利追蹤收斂狀態。
- 整體指標（以各折測試預測合併計算）：Accuracy 0.9708、Precision 0.9676、Recall 0.9952、Specificity 0.8906、F1 0.9812、Balanced Accuracy 0.9429。

| Fold  | TN | TP | FP | FN | Accuracy | Precision | Recall | F1     | Balanced Acc |
|-------|----|----|----|----|----------|-----------|--------|--------|--------------|
| Fold0 | 35 |125 |4   |1   |0.9697    |0.9690     |0.9921  |0.9804 |0.9447        |
| Fold1 | 33 |126 |6   |0   |0.9636    |0.9545     |1.0000  |0.9767 |0.9231        |
| Fold2 | 34 |125 |4   |1   |0.9695    |0.9690     |0.9921  |0.9804 |0.9434        |
| Fold3 | 34 |126 |4   |0   |0.9756    |0.9692     |1.0000  |0.9844 |0.9474        |
| Fold4 | 35 |125 |3   |1   |0.9756    |0.9766     |0.9921  |0.9843 |0.9566        |

跨折結果顯示 Precision 與 Recall 均維持在 95% 以上，且 Balanced Accuracy 落在 0.92–0.96 之間，證實最佳模型在 39 維特徵上的泛化能力。訓練時每折自動計算類別權重（良品 ≈2.14、壞品 ≈0.65），並於 50–70 個 epoch 內收斂。
